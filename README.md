...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/UI%20Style-Apple%20Frosted%20Glass%20v2.0-blue.svg" alt="Apple Frosted Glass v2.0">
...
### 核心架构构想与双轨回答机制
系统对标**广州市人民政府门户网站（`www.gz.gov.cn`）**政策库与**广东政务服务网（`www.gdzwfw.gov.cn`）**全生命周期办事服务指引，建立中型政务服务数据库，实现 AI 双轨精准答复：
1. **当用户询问政策时**：
   - 提取法条核心要点进行大白话解读；
   - 严格挂载**【官方政策依据】**红头公文卡片，展示正式发文字号（如`穗公积金规字〔2023〕1号`、`穗府办规〔2022〕17号`等）、制定机关、施行日期，并支持一键展开条款原文抽屉，保证 100% 权威可溯源，杜绝大模型事实性幻觉；
2. **当用户询问如何办理/办事流程时**：
   - 以**【办事向导结构化清单】**纯净专业格式输出：
     - **【准入条件】**：清晰罗列硬性门槛（户籍、社保月数、学历/年龄等），无任何套话尾注，便于市民精准自检；
     - **【申报材料清单】**：纯净清爽展示所需材料，彻底剔除冗余后缀徽章与杂音说明，一目了然；
     - **【办理流程与渠道】**：
       - **承诺时限与网点**：规整横向对齐展示法定办结时限与线下综合窗口；
       - **办理流程环节**：以规范有序的数字步骤（`1. 环节名：办理说明`）清晰递进，彻底剔除耗时杂音括号；
       - **【官方在线申办直达】**：优雅克制的官方申报通道按钮，携带对应事项唯一实施编码与**广东政务服务网具体服务页面的真实在线申报跳转链接**，杜绝大段搜索套话，市民点击即可直达官方网办页面；
3. **苹果级毛玻璃美学与人本化自然对话（v2.0.0 极致简约重塑）**：
   - **半透明毛玻璃材质（Glassmorphism）**：窗口容器与气泡采用 iOS/macOS 级别的高级毛玻璃滤镜（`backdrop-filter: blur(28px) saturate(190%)`），配合通透白底（`rgba(255, 255, 255, 0.78)`）、极细微光边缘（`rgba(255, 255, 255, 0.75)`）与平滑圆角（Apple Squircle），呈现轻盈现代的高级视觉质感；
   - **彻底净化视觉噪点**：移除了顶部机械式的通告横幅、底部冗余版权注脚、欢迎语中的繁杂提示与示例列表，仅保留最纯粹核心的对话空间；
   - **人本化自然对话风格**：彻底摒弃传统机械式“能办！”、“可以！”等感叹号叫喊式回复，转而采用温和、客观、优雅而克制的自然语言（Apple Intelligence 风格），直接交代核心政策红利与办理条件；
   - **官方政策依据轻量单行注脚**：在气泡左下角以规整的单行小字呈现（`政策依据：《...》（发文字号） · 查看条文原文 ▾`），点击可平滑展开纯净灰底法条抽屉查验原文，无色块打扰，沉浸阅读；
