...
3. **严肃规范的政务视觉与极简公文排版（v1.9.1 彻底摒弃扎彩色块与红框，左下角单行轻量政策依据脚注）**：
   - **杜绝刺眼色块拼贴**：彻底废弃深蓝大横幅、深灰条、深绿背景、橙色块、黄色警报大块、以及政策依据底色块与粗红边等“调色盘”式色块堆叠；在浅色与暗色模式下均呈现纯粹沉稳、通透耐看的高级公文纸排版；
   - **强化排版质感与层级节奏**：统一为洁净白底、极细灰色分割线（`#e2e8f0`）、标准中文字号与 1.65 舒适行距；
   - **官方政策依据轻量单行注脚**：完全摒弃原有的厚重大色块与粗红边卡片，转而在气泡左下角以规整的单行小字呈现（`政策依据：《...》（发文字号） · 查看条文原文 ▾`），点击可平滑展开纯净灰底法条抽屉查验原文，无色块打扰，阅读体验沉浸丝滑；
