...
### 4.4 答复结构极简重构与降噪原则（v1.9.1 纯进化）
在研发与高频评测过程中，系统深入分析了群众办事咨询的认知负荷，确立了**“去粗取精、杜绝形式主义、少用色块、优化输出格式”**的重构原则：
1. **彻底消除非结构化文本与步骤卡片的重复冗余**：原先的【办事要点】文本列表与向导卡片存在 100% 内容重叠，系统直接以【办事向导结构化清单】高保真排版取代原有散乱文本；
2. **全面剔除“形式主义车轱辘废话”**：
   - 彻底删除步骤 1 准入底部的“请对照上述准入条件确认是否符合申报资质...”指导语尾注；
   - 彻底删除步骤 2 材料底部的“核心材料已接入广州政务大数据联网核验...”重复说明；
   - 彻底删除材料项后面冗余的绿底小徽章（如“电子证照免提交”、“系统自动核验”），还原纯粹清爽的材料列表；
   - 彻底删除步骤 3 各流程环节中的“（预计耗时：x分钟）”杂音括号小字，仅保留清晰的阶段动作；
   - 彻底删除线上办理路径中的模板化套话（“打开手机微信搜索“穗好办”小程序或登录广东政务服务网广州站...完成人脸识别实名认证后在线确认申报即可”），直接外露醒目的官方直达链接；
   - 彻底清除底部的“信息承办：广州市政务服务和数据管理局”冗余行政注脚；
3. **彻底摒弃扎眼的大面积饱和色块拼贴**：
   - 坚决废弃蓝色大横幅、深灰条、深绿背景、橙色块、黄色警报大块等“调色盘”堆砌，在浅色与深色模式下均呈现纯净、通透、耐看的高级公文纸排版；
   - **政策依据彻底移除厚重色块与粗红线**：原有的红黄渐变大卡片被彻底替换为气泡左下角极简单行小字脚注（`政策依据：《...》（发文字号） · 查看条文原文 ▾`），不仅消除了色块的突兀感，更将视觉焦点完全归还给核心答复与办事向导；点击“查看条文原文”时才按需展开极简纯灰底法条抽屉查验公文细则；
   - 依靠字重、字号与间距构建天然层次，而非粗暴堆积高对比度色块；
4. **剔除【政务知识图谱关联】卡片**：知识图谱三元组与紧邻的【官方政策依据】公文溯源高度重合，且学术化的三元组增加了普通老百姓办事的认知障碍，因此全面剔除知识图谱前端冗余展示；
5. **取消底部多次点击交互**：完整向导清单随首轮答复直接展开，市民无需再逐次点击 3 个按钮生成重复气泡。
...
* **极简低噪公文配色准则**：
  * 广州政务深蓝 (`#0050b3` / `#003a8c`)：用于窗口顶栏、标题文字加粗与官方徽标；
  * 纯净白底与浅灰衬底 (`#ffffff` / `#f8fafc`)：卡片主体统一采用纯净底色，彻底废弃刺眼的彩色大背景色块；
  * 极细分割线 (`#e2e8f0` / `#f1f5f9`)：运用细边框和虚线分割代替沉重背景，保持排版轻盈呼吸感；
