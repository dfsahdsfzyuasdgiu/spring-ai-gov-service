...
> **建设标准**：对齐全国一体化在线政务服务平台标准规范、广东省“粤省事/粤商通”、广州市“穗好办”移动政务服务规范，融合苹果级半透明毛玻璃美学（Glassmorphism）与人本化自然对话语言，极致简约降噪，严格 0 Emoji。  
...
- [4. 核心业务逻辑与技术实现](#4-核心业务逻辑与技术实现)
  ...
  - [4.4 答复结构极简重构与人本化自然对话 (v2.0.0 极致纯粹演进)](#44-答复结构极简重构与人本化自然对话-v200-极致纯粹演进)
  - [4.5 办事向导结构化清单与广东政务服务网在线申办直达](#45-办事向导结构化清单与广东政务服务网在线申办直达)
  - [4.6 广州市政务公文爬虫采集服务 (Crawler Engine)](#46-广州市政务公文爬虫采集服务-crawler-engine)
- [5. 前端 UI/UX 设计与苹果级毛玻璃美学](#5-前端-uiux-设计与苹果级毛玻璃美学)
  - [5.1 苹果级半透明毛玻璃视觉体系 (Apple Glassmorphism Aesthetic)](#51-苹果级半透明毛玻璃视觉体系-apple-glassmorphism-aesthetic)
  ...
  - [5.5 窗口最小尺寸约束与无极缩放架构设计](#55-窗口最小尺寸约束与无极缩放架构设计)
...
### 4.4 答复结构极简重构与人本化自然对话（v2.0.0 极致纯粹演进）
在研发与高频评测过程中，系统深入贯彻“极致简约美，仅保留最核心功能与语言，对齐 Apple 级产品质感”的重构原则：
1. **人本化自然表达，彻底淘汰机械式叫喊**：
   - 传统政务问答模型常生硬套用“能办！”、“可以！”、“不能办！”等感叹号开头，具有强烈的机械模板痕迹；
   - v2.0.0 全面重构大模型系统提示词与回复范式，采用自然、客观、优雅而克制的自然语言（Apple Intelligence 风格），直接陈述市民享有的核心权益、政策红利或准入门槛（如“在广州稳定就业且无自有产权住房的新就业职工，符合条件每月最高可领取 1,400 元住房租赁补贴...”）；
2. **彻底清除视觉杂音与形式主义文案**：
   - 彻底删除顶部机械化的通知横幅（`[通告] 依托广州市政策公文数据库与政务知识图谱...`）；
   - 彻底删除底部冗余的官方注脚横幅（`广州市人民政府门户网站 · 政策法规智能问答专窗 | 12345 便民热线协同`）；
   - 彻底净化初始欢迎语，剔除冗杂的预设问题列表与数据源注脚，仅保留温和亲切的简练问候；
   - 彻底删除步骤 3 中当已展示直达按钮时重复出现的搜索套话（“打开手机微信搜索‘穗好办’小程序或电脑登录广东政务服务网...”）；
   - 彻底删除材料后面所有的冗余徽章、耗时括号、申报资质确认套话与承办单位注脚；
3. **半透明毛玻璃材质重构**：
   - 全面引入苹果级高斯模糊滤镜（`backdrop-filter: blur(28px) saturate(190%)`）与半透明白色图层（`rgba(255, 255, 255, 0.78)`）；
   - 搭配柔和微光边框与平滑圆角（Squircle），彻底告别老旧生硬的彩色色块拼贴；
4. **官方政策依据轻量单行注脚**：
   - 在气泡左下角以规整单行小字呈现（`政策依据：《...》（发文字号） · 查看条文原文 ▾`），点击按需平滑展开抽屉查验公文细则；
5. **取消底部多次点击交互**：
   - 完整向导清单随首轮答复直接呈现，免除多次点击重复生成气泡。

### 4.5 办事向导结构化清单与广东政务服务网在线申办直达
1. **第一层：【快速答疑】**
   - 1-2 句核心定性结论，开门见山直击市民关切（“到底能不能办”、“能领多少补贴/最高1400元”、“最长可以领5年/最快1个工作日办结”）；
2. **第二层：【办事向导结构化清单】（纯白底、细线分割、极简公文文档排版）**
   - **【准入条件】**：清晰罗列硬性准入门槛（户籍、社保月数、学历/年龄等），群众对照即可自检；
   - **【申报材料清单】**：纯净清爽展示材料清单（▪ 居民身份证与广东省居住证原件），无任何视觉杂质与多余标签；
   - **【办理流程与渠道】**：
     - **承诺时限与网点**：顶部横向对齐展示法定承诺办结时限与线下综合受理窗口；
     - **流程阶段拆解**：规范有序的数字步骤列表（`1. 环节名：办理说明`），逻辑严谨连贯；
     - **【官方在线申办直达】**：优雅专业的官方申报通道超链接，携带对应事项唯一实施编码（如 `GZ-LS-JFRH002`）与**广东政务服务网具体服务页面的真实在线申报跳转链接**，市民点击即可直达官方网办页面；
     - **注意事项**：轻量灰底、左侧细警戒线，杜绝刺目的大黄块。
3. **第三层：【官方政策依据】（左下角单行极简文字注脚）**
   - 彻底摒弃色块卡片，在左下角规整呈现红头文件名称与文号（`政策依据：《...》（文号）`），配备纯文字链接“查看条文原文 ▾”，点击按需展开清爽的法规原文抽屉；
4. **第四层：【相关政策延伸咨询】**
...
## 5. 前端 UI/UX 设计与苹果级毛玻璃美学

### 5.1 苹果级半透明毛玻璃视觉体系 (Apple Glassmorphism Aesthetic)
* **毛玻璃材质与多层通透光感**：
  * 主面板容器与浮层全面采用 iOS / macOS 设计语言：`backdrop-filter: blur(28px) saturate(190%)` 与 `-webkit-backdrop-filter`；
  * 优雅通透半透明白底：主窗体 `rgba(255, 255, 255, 0.78)`、顶栏/底栏 `rgba(255, 255, 255, 0.65)`、AI 消息气泡 `rgba(255, 255, 255, 0.82)`；
  * 高级微光半透明边框：统一采用 `1px solid rgba(255, 255, 255, 0.75)`；
  * 漫反射微阴影：`box-shadow: 0 16px 40px rgba(0, 0, 0, 0.12), 0 2px 8px rgba(0, 0, 0, 0.05)`，层次自然深邃；
* **连续曲率平滑圆角 (Apple Squircle Geometry)**：
  * 主窗口圆角 `border-radius: 18px`，消息气泡 `border-radius: 16px`；
  * 快捷提问药丸标签与输入框 `border-radius: 20px`（Pill-shaped）；
  * 顶栏控制按钮圆角 `border-radius: 50%`（圆形轻量微晶按钮）；
* **人本配色与纯粹信息层级**：
  * 用户消息气泡：采用经典 Apple iMessage 优雅蓝色渐变（`linear-gradient(135deg, #0071e3 0%, #0056b3 100%)`）；
  * 字体栈：优先调用 `-apple-system, BlinkMacSystemFont, "SF Pro Text", "PingFang SC", "Segoe UI"` 等顶级现代字体；
  * 彻底净化色彩污染：坚决摒弃突兀的大红大绿背景块，依靠文字字重（`font-weight: 590 / 600`）与自然间距构建纯粹的信息节奏。

### 5.2 三重零 Emoji 纪律防御体系 (Triple-Layer Zero Emoji Standard)
在严肃政务场景下，任何卡通表情（如 ❗、✅、⚠️、📅、📌 等）都会严重破坏政府公信力。系统构建了三重防御体系确保全局 Emoji Count 严格为 0：
1. **第一层（LLM System Prompt 纪律约束）**：提示词明文禁止大模型输出任何 Emoji 表情或彩色符号，列表强制使用纯文字或简明破折号；
2. **第二层（Java 后端 SSE 流式字符过滤）**：`GovAiService` 在流式发送 chunk 及落库前，调用 `stripEmoji()` 扫描 Unicode 字符区间（涵盖 Dingbats、变体选择符等）实时脱敏；
3. **第三层（前端 Shadow DOM 转义拦截）**：JavaScript 底层 `escapeText()` 和 `formatMarkdownLike()` 统一注入正则清洗，任何外部文本进入 DOM 前均自动洗脱表情符号。
