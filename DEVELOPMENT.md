...
### 4.4 答复结构极简重构与降噪原则（v1.8.0 纯进化）
在研发与高频评测过程中，系统深入分析了群众办事咨询的认知负荷，确立了**“去粗取精、杜绝形式主义、一目了然”**的重构原则：
1. **彻底消除非结构化文本与步骤卡片的重复冗余**：原先的【办事要点】文本列表与向导卡片存在 100% 内容重叠，系统直接以【办事向导三步法】高保真卡片取代原有散乱文本；
2. **全面剔除“形式主义车轱辘废话”**：
   - 彻底删除步骤 1 准入卡片底部的“请对照上述准入条件确认是否符合申报资质...”指导语尾注；
   - 彻底删除步骤 2 材料卡片底部的“核心材料已接入广州政务大数据联网核验...”重复说明；
   - 彻底删除材料项后面冗余的绿底小徽章（如“电子证照免提交”、“系统自动核验”），还原纯粹清爽的材料列表；
   - 彻底删除步骤 3 各流程环节中的“（预计耗时：x分钟）”杂音括号小字，仅保留清晰的阶段动作；
   - 彻底删除线上办理路径中的模板化套话（“打开手机微信搜索“穗好办”小程序或登录广东政务服务网广州站...完成人脸识别实名认证后在线确认申报即可”），直接外露醒目的官方直达链接卡片；
   - 彻底清除卡片底部的“信息承办：广州市政务服务和数据管理局”冗余行政注脚；
3. **剔除【政务知识图谱关联】卡片**：知识图谱三元组与紧邻的【官方政策依据】公文溯源高度重合，且学术化的三元组增加了普通老百姓办事的认知障碍，因此全面剔除知识图谱前端冗余展示；
4. **取消底部多次点击交互**：完整向导三步法随首轮答复直接展开，市民无需再逐次点击 3 个按钮生成重复气泡。

### 4.5 办事向导三步法与广东政务服务网在线申办直达
1. **第一层：【快速答疑】**
   - 1-2 句核心定性结论，开门见山直击市民关切（“到底能不能办”、“能领多少补贴/最高1400元”、“最长可以领5年/最快1个工作日办结”）；
2. **第二层：【办事向导三步法】（蓝/橙/绿分阶结构化卡片，高信息密度）**
   - **步骤 1【准入资格自查】（政务蓝 `#1677ff`）**：清晰罗列硬性准入门槛（户籍、社保月数、学历/年龄等），群众对照即可自检；
   - **步骤 2【申报材料清单】（便民橙 `#fa8c16`）**：纯净清爽展示材料清单，无视觉杂质；
   - **步骤 3【全流程办事指引】（通畅绿 `#389e0d`）**：
     - **承诺办结时限**：浅绿药丸徽章明确办结工作日；
     - **流程阶段拆解**：白底独立行清晰展示第1步、第2步、第3步操作环节（纯净无耗时噪点）；
     - **线上办理通道与【官方在线申办直达】**：步骤 3 底部直出醒目的官方申报直达卡片，携带对应事项唯一实施编码（如 `GZ-GJJ-ZFTQ008`）与**广东政务服务网具体服务页面的真实在线申报跳转链接**，市民点击即可直达官方网办页面；
     - **线下办事网点**：独立灰色标识栏明确指定实体大厅与综合窗口地址；
     - **注意事项与避坑提醒**：以琥珀黄独立警示框高亮社保断缴、申请时间截点等高频误区。
...
* **广州政务配色与色彩分阶体系**：
  ...
  * 准入自查蓝 (`#1677ff` / `#f0f7ff`)：应用于步骤 1【准入资格自查】，代表权威资质审核；
  * 申报材料橙 (`#fa8c16` / `#fff8f0`)：应用于步骤 2【申报材料清单】，代表温馨材料准备；
  * 通畅绿 (`#389e0d` / `#237804` / `#f6ffed`)：应用于步骤 3【全流程办事指引】，代表全流程绿灯速办；
